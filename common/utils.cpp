...
void GetAbsTimeInFuture(struct timespec* tspec, uint64_t ms) {
  ...
  uint64_t timens = now.tv_usec * 1000 + ms * 1000000L;
  tspec->tv_sec = now.tv_sec + (timens / 1000000000L);
  tspec->tv_nsec = timens % 1000000000L;
}
...
uint64_t GetRandom(uint64_t max_val) {
